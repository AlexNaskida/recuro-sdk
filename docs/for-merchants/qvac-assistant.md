...
- *"Which plans have the highest churn? What should I change?"*
- *"How can I attract more customers?"*
- *"Which subscribers are at risk of churning?"*
- *"What's my best-performing plan and why?"*
...
Follow the QVAC install guide (see your QVAC docs). Confirm it's running:
...
Set `QVAC_MODEL` in `client/apps/merchant-dashboard/src/lib/config.ts` to the model name your QVAC runtime exposes. Larger models (8B+) give noticeably better extraction of plan parameters from natural language.
...
Look for the **green online indicator** on the bot avatar in the chat header — that confirms the dashboard reached the QVAC runtime.
