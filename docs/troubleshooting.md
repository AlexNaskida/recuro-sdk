...
You're probably querying with the wrong merchant pubkey. `fetchMerchantPlans` only returns plans where `merchant === passed pubkey`. The `merchantReceiveAddress` is a *different* field (the payout destination).
...
Check the *USDC ATA*, not SOL. `InsufficientBalance` only refers to the stablecoin token account associated with the configured stablecoin mint.
