...
| **Guard PDA**         | A per-subscription escrow-like authority. Holds the immutable transfer parameters and is the *actual* delegate of the subscriber's USDC. |
...
| **`amountUsdc`**      | At the SDK layer, the *human* number (e.g. `9.99`). On-chain it's stored as micro-USDC.                                                  |
